...
public interface PatientRepository extends JpaRepository<User, Long> {
    ...
    @Query(value = "SELECT u.id, u.first_Name, u.last_Name, u.date_Of_Birth, u.email,\n" +
            "            u.phone_Number, u.address, h.id as hospital_Card_Id, h.preliminary_diagnosis as diagnosis , h.discharged as discharged, h.doctor_Id as doctor_Id, d.first_Name as doctor_First_Name, d.last_Name as doctor_Last_Name, c.name_Category as doctor_Name_Category FROM users as u \n" +
    ...
    List<PatientWithHospitalCardAndDoctor> findAllWithHospitalCardsAndDoctors();
    ...
    List<PatientWithNumberOpenedHospitalCards> findAllWithNumberOpenedHospitalCards();
}
